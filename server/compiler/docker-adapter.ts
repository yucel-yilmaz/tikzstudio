...
import type { CompilerAdapter, CompileInput, CompileResult } from "@/server/compiler/types";
...
export class DockerCompilerAdapter implements CompilerAdapter {
  async compile(input: CompileInput): Promise<CompileResult> {
    ...
    if (sourceBytes > env.MAX_SOURCE_SIZE_KB * 1024) {
      throw new AppError("VALIDATION_ERROR", "Source exceeds allowed size.", 400, {
  ...
  private async runDocker(args: string[]) {
    return new Promise<{ code: number | null; log: string; timedOut: boolean }>((resolve) => {
