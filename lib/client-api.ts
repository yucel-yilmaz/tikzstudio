...
export function getProjectFiles(projectId: string) {
  return requestJson<{ files: ProjectFileDto[] }>(`/api/projects/${projectId}/files`);
}

export function updateFile(projectId: string, fileId: string, input: UpdateFileInput) {
  return requestJson<ProjectFileDto>(`/api/projects/${projectId}/files/${fileId}`, {
    method: "PATCH",
    body: JSON.stringify(input),
  });
}

export function compileProject(projectId: string, payload?: { engine?: string; mainFileId?: string }) {
